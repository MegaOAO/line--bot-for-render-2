...
from modules.reply import faq, menu ,horoscope
from modules.currency import get_exchange_table
from modules.fortune import get_fortune
from modules.lots import get_lots
...
def handle_message(event):
    with ApiClient(configuration) as api_client:
        ...
        bot_msg = TextMessage(text="請輸入指令「選單」以使用功能。")
        if user_msg in faq:
            bot_msg = faq[user_msg]
        elif user_msg in ["選單"]:
            bot_msg = menu
        elif user_msg in horoscope:
            ans = get_fortune(type_id= user_msg)
            title = ans[0]["星座"]
            time = ans[0]["日期"]
            yi = ans[0]["今日宜做"]
            ji = ans[0]["今日不宜做"]
            ...
            money = ans[0]["金錢運勢"]
            health = ans[0]["健康運勢"]
            luckycolor = ans[0]["幸運色"]
            bot_msg = TextMessage(text=f"{title} {time}\n\n宜:{yi}\n不宜:{ji}\n\n今日幸運色:{luckycolor}\n\n今日整體運勢:\n{all}")
        elif user_msg in ["抽籤"]:
            draw_lots = lots[0]
            bot_msg = TextMessage(text = f"您抽到的籤是:{draw_lots}")
